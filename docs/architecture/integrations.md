...
P2：DocumentParser 的 TextParser 实现 TXT/MD；SqliteEvidenceStore 使用 Node 内置 SQLite 与独立 evidence.db，实际 runtime FTS5 probe 和 literal fallback 测试通过。公开 defineTool / systemPrompt.section 挂载 retrieval 工具与静态 grounding policy；真实 packed Host 的 registry dispatch 和 prompt assembly 已验收。当前没接入 Client extension。
