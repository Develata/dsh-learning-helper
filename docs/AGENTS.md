...
一个事实一个 owner，以链接引用，禁止复制规则、聊天转储、标题占位和全量 symbol dump。当前没有 generated docs；`.codegraph/` 是本地生成索引，不提交。发现冲突按表确定语义 owner，并同步矛盾处。最终验收状态用 planned / verified / deferred / blocked；verified 必须关联实际命令与结果，blocked 写明外部条件，不能以 fixture 结果替代真实模型行为。
