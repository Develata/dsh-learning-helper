...
真实模型验收需要先在 Harness 配置可用 LLM 凭证（不要提交 key），通过安装本插件的 Web 会话提问“为什么闭区间上的连续函数一定一致连续？”，检查日志实际出现 search/read，答案的 machine references 必须来自该次 read。P2 本机只有 browser-session credential，无模型 credential；未执行该 semantic smoke。操作顺序与判据见 [golden path](../acceptance/golden-path.md)。
