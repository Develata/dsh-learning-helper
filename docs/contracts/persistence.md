# Persistence contract v0.2（学习 schema 语义不变）

学习真值仍为 LearningAggregate，每 Workspace 恰好一个内部 Course。v0.2 使用 `.learning-helper/state.db` 自有 SQLite schema2 单行 aggregate；Harness 没有稳定的 workspace-local storage-domain composition。旧 domain version1 保留在显式迁移适配器，不在运行时创建全局 courses table。manifest projectId 与 Course.id 必须一致；unknown version/corruption 拒绝，不重置。详情见 [ADR-0005](../adr/0005-workspace-learning-boundary.md)。
...
Idempotency：submissionId 项目内唯一；同 ID 同 quiz/相同选项重试返回既有 receipt，无重复 Attempt/revision；同 ID 不同内容冲突；相同 quiz 用另一 ID 重提冲突。并发通过 provider 有界32写队列和 SQLite atomic update 串行化，单 Host 进程拥有该 DB；不支持多 Host 同写。
...
资源上限：单 Workspace 200 quizzes / 4000 attempts / 100 plan revisions；超限明确拒绝，绝不静默删学习证据。令 N 为聚合大小、C 为概念数、A 为 Attempt 数；当前校验/策略包含按概念扫描 Attempts，时间 O(N + C×A)、空间 O(N)，C ≤ 100。限额内优先一致性；未来扩大规模需新 ADR 与迁移。
