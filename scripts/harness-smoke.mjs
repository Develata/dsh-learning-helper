...
try {
  ...
  try {
    ...
    if (process.env.LH_BROWSER_SMOKE !== '0') {
      const options = { web: second, harness, plugin, work, screenshotsPath: process.env.LH_BROWSER_SCREENSHOTS };
      browserReceipt = process.env.LH_DEMO_OUTPUT
        ? await (await import('./record-demo.mjs')).recordDemo({ ...options, output: resolve(process.env.LH_DEMO_OUTPUT) })
        : await browserSmoke(options);
      before = await (await second.get(path(browserReceipt.sessionId) + '/dashboard')).json();
      assert.equal(before.currentPlan.version, 2);
      receipts.push(process.env.LH_DEMO_OUTPUT
        ? 'Chromium recording: workspace initialization, MD/PDF upload, fixture tool authoring, mathematical quiz, Weak/v2 and refresh'
        : 'Chromium: workspace initialization, MD/PDF upload, grounded authoring, quiz retry, Weak/v2, task sessions, workspace switch');
    }
